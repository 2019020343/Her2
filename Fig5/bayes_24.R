...
for (i in 1:10000) {
  ...
  roc1<- roc(test[,ncol(test)],as.numeric(as.matrix(prediction)))
  roc<-c(roc,roc1$auc)
  acc<-c(acc,acc1)
  print(i)
...
max(acc)
max(roc)
write.table(roc,"D:\\her2\\fig\\5\\ROC\\F8_bayes_ROC.txt",col.names = T, row.names = F,sep = "\t" ,append = FALSE, quote = F)
roc<- read.table("D:\\HER2\\fig\\5\\ROC\\F8_bayes_ROC.txt",header = T,sep = "\t")
